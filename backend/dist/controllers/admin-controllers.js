...
const getAllOrders = (req, res, next) => {
  ...
  db_1.default.query(
    admin_queries_1.getAllOrdersQuery,
    [+limit, offset],
    (error, results) => {
      if (error) {
      ...
      db_1.default.query(
        ...
        (countError, countResult) => {
          if (countError) {
            return next(
              (0, error_1.default)(500, "Error fetching orders count")
            );
          }
...
const updateOrderStatus = (req, res, next) => {
  ...
  db_1.default.query(
    admin_queries_1.updateOrderStatusQuery,
    [status, +orderId],
    (error, results) => {
      if (error) {
...
const addBook = (req, res, next) => {
  ...
  db_1.default.query(
    ...
    (error, results) => {
      ...
      db_1.default.query(
        admin_queries_1.addGenresToBookQuery,
...
const updateBook = (req, res, next) => {
  ...
  db_1.default.query(
    admin_queries_1.updateBookQuery,
    [
      +bookId,
      title,
      author,
    ...
    (error, results) => {
      ...
      db_1.default.query(
        admin_queries_1.updateGenresToBookQuery,
        [+bookId, genres],
        (error, results) => {
          if (error) {
...
const deleteBook = (req, res, next) => {
  const { bookId } = req.params;
  db_1.default.query(
    admin_queries_1.deleteBookByIdQuery,
    [+bookId],
    (error, results) => {
      if (error) {
...
const getGenres = (req, res, next) => {
  db_1.default.query(admin_queries_1.getGenresQuery, (error, results) => {
    if (error) {
      return next(
        (0, error_1.default)(500, "We could fetch genres. Try again later.")
      );
    }
